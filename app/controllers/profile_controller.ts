...
export default class ProfileController {
    ...
    public async getProfile({ user, response }: HttpContext): Promise<void> {
    ...
    public async acceptTermsAndConditions({ user, response }: HttpContext): Promise<void> {
        user.acceptedTermsAndConditions = true;
        await user.save();
        await user.refresh();

        return response.send({ accepted: true });
    }
}
